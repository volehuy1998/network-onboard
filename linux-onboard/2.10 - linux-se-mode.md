...
Chính sách tiêu chuẩn truy cập gọi là `Discretionary Access Control (DAC)` mô tả sự cho phép dựa trên người dùng, nhóm, quyền truy cập. `DAC` là không cung cấp cho quản trị viên tạo các chính sách bảo mật toàn diện và chi tiết mà ngược lại nó mô tả một không gian quyền hạn tương đối rộng lớn, mơ hồ và đây chính là điểm yếu, nó giúp cho những người dùng được ủy quyền sử dụng với mục đích khác cho dù là vô tình hay cố ý. `SELinux` là một tính năng quan trọng bù đắp cần thiết cho `DAC` và chính sách khác. Việc truy cập vào tệp, cổng và tài nguyên khác sẽ được `SELinux` kiểm soát ở mức độ chi tiết. 
...
Khi dùng lệnh `mv` di chuyển tệp tin sang nơi khác thì nó cũng sẽ mang thuộc theo thuộc tính `SELinux` nhưng nếu dùng lệnh `cp` thì tệp tin sẽ mang thuộc tính `SELinux` mà đã được chính sách định nghĩa.
...
Mặc định `SELinux` sẽ ngăn `httpd` truy cập vào thư mục `UserDir` của người dùng thông qua `httpd_enable_homedirs`. Kích hoạt chích sách sẽ cho phép sử dụng `UserDir` chia sẻ lẫn nhau. 
