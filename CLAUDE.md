...
9. **No grandfather clause.** Earlier transcripts (R0 baseline, R1.A apt-distro-install) were captured before this no-synthetic-structure rule was codified and originally contained `echo === ... ===` start-end markers plus `echo --- ... ---` section banners. The owner directive of 2026-04-29 ("Use words to explain instead of this stuff") retracts the prior R1.A grandfather and applies the no-synthetic-structure rule to every lab `.md` file, including R0 and R1.A. The fix is .md-only: every banner-and-echo line pair is removed from the curriculum-side fenced block and replaced with a prose paragraph. The typescript files themselves are not modified, because the typescript is the verbatim historical record of what was actually run on the lab host; the curriculum file is what readers see, and it is the file the rule governs. `lab_verbatim_check.py` is unaffected: removing lines from the .md is allowed (the check only fails when an .md line is not present in the typescript, never the reverse).
