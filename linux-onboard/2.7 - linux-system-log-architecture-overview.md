...
- [2.7 - Kiến trúc nhật ký hệ thống (UPDATED 10/12/2023)](#sys_log_arch)
  ...
  - [2.7.4 - Đồng bộ thời gian ( :heavy_plus_sign: UPDATED 10/12/2023)](#clock_time)
    - [2.7.4.1 - Tổng quan `Network Time Protocol` ( :heavy_plus_sign: UPDATED 10/12/2023)](#ntp)
    - [2.7.4.2 - Công cụ `datetimectl` ( :heavy_plus_sign: UPDATED 10/12/2023)](#datetimectl)
    - [2.7.4.3 - Cấu hình `NTP` sử dụng `chrony` ( :heavy_plus_sign: UPDATED 10/12/2023)](#chronyd)
    - [2.7.4.4 - Cấu hình `NTP` sử dụng `ntpd` ( :heavy_plus_sign: UPDATED 10/12/2023)](#ntpd)
...
`chronyd` cùng là dịch vụ đồng bộ thời gian được triển khai dựa trên `Netwỏk Time Protôcl (NTP)`. Về mặc định `chronyd` sử dụng máy chủ từ `Pool NTP Project` để đồng bộ hóa thời gian mà không cần yêu cầu cấu hình. Nếu không được cung cấp truy cạp mạng toàn cầu thì quan trị viên cần thay đổi địa chỉ máy chủ `NTP`. Từ các phiên bản `CentOS 6` trở về trước sử dụng `ntpd` nhưng các phiên bản sau đã thay thế bằng `chronyd`.
...
Cấu hình `chronyd` tại `/etc/chrony.conf` như sau:
...
Đồng bộ lại thời gian, tùy chọn `-v` để xem chi tiết thông tin như sau:
...
- `^` đang tham chiếu đến máy chủ `NTP` ngoài mạng toàn cầu. Nếu `#` là máy chủ `NTP` cục bộ, ngược lại `=` là đồng bộ từ thiết bị ngnag cấp `stratum`.
- `ntp1.toni.org`, ... là địa chỉ của máy chủ `NTP`.
- Giá trị bát phân `377` chỉ ra rằng nhận được phản hồi hợp lệ cho tất cả tám lần truyền cuối cùng.
- `LastRx` cho biết mẫu tín hiệu cuối cùng nhận được cách đây bao lâu, mặc định là giây cũng có thể hiển thị phút `m`, giờ `h`, ngày `d` hoặc năm `y`
