...
### Block XX, Operational Excellence (Part 20, 4 file)
...
- **Part 20.2**, [OVN troubleshooting deep-dive](20.2%20-%20ovn-troubleshooting-deep-dive.md) *(content, Session S51 Phase G.3.1)*, 3 lớp debug OVN (NB intent → SB Logical_Flow → OpenFlow `br-int`), `ovn-trace` 11 option + 4 output mode (`--detailed`/`--summary`/`--minimal`/`--all` + `--ovs`/`--ct`/`--lb-dst`/`--select-id`/`--friendly-names`/`--db`) + 5 class microflow, chain `ofproto/trace | ovn-detrace` map cookie → Logical_Flow → NB object (ACL/LB/NAT), Port_Binding 8 type forensic (10 failure pattern với diagnostic chain), `ovn-appctl -t ovn-controller` 11 command + `ovn-appctl -t ovn-northd` 10 command (Anatomy Template A cho 7 command key), MAC_Binding + FDB + Service_Monitor stateful triage, 16-symptom diagnostic matrix, 3 Guided Exercise + 1 Capstone POE.
- **Part 20.3**, [OVN daily operator playbook](20.3%20-%20ovn-daily-operator-playbook.md) *(content, Session S53 Phase G.5.1)*, 10 task category scenario-driven cho daily workflow: (1) health check morning routine 5 lệnh < 10 giây, (2) inventory listing đếm LS/LSP/LR/ACL/LB/NAT/DHCP/Chassis, (3) port lifecycle 6 scenario (add/bind/remove/rename/migrate/disable), (4) ACL management 6 scenario với Port_Group consolidation, (5) Load_Balancer + NAT 6 scenario, (6) DHCP + DNS native 5 scenario, (7) Gateway + HA_Chassis 5 scenario, (8) Conntrack 5 scenario, (9) Performance 5 metric, (10) Backup + maintenance 5 scenario với chassis cordon 7-step. **2 workflow end-to-end**: new tenant provisioning script + tenant teardown script. **3 Guided Exercise** + **1 Capstone POE** ("Add 500 ACL safe for prod?" refute với Port_Group recommendation). Anatomy Template A cho 10+ command output.
