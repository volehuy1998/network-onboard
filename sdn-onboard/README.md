...
- Part 0.4, [Open vSwitch installation, three paths compared](0.4%20-%20ovs-installation-three-paths.md) *(new 2026-04-29, 579 lines)*: pedagogical comparison of the three install paths captured on `lab-openvswitch` (path A apt distro, path B release tarball, path C git checkout). Five-concern trade-off matrix (build flag control, source freshness, kernel-module strategy, install layout, operational overhead), three common failure modes (out-of-tree kernel-module rejection on 5.9+, residual database after `apt-get purge`, persisted kernel module), recovery procedures, and a five-step decision tree for picking the right path. Pairs with the three verbatim R1 lab transcripts under `labs/v3.13-R1{A,B,C}-*.md` and the autotools theory companion at `source/v3.13-S0-build-system-and-code-map.md`.
- Part 0.5, [OVS bridge and port primitives, hands-on synthesis](0.5%20-%20ovs-bridge-and-port-primitives.md) *(new 2026-04-30, 802 lines)*: curriculum wrap of plan v3.13 R2 sub-captures (R2.0 through R2.8). Walks the bridge lifecycle (`add-br`, `del-br`), port classes (`internal`, `system`, `patch`), `ofport` versus datapath port number, `dpif/show` interpretation, MAC learning behaviour (`mac-aging-time` floor at 15 seconds, `mac-table-size` cap), bond, mirror, and the patch-pair pattern. Pairs with the eight R2 lab transcripts.
- Part 0.6, [Traditional network models versus OVS, comparative architecture](0.6%20-%20traditional-vs-ovs-comparative-architecture.md) *(new 2026-04-30, 306 lines)*: closing wrap of plan v3.13 R2.5 sprint (eleven sub-captures R2.5.1 through R2.5.11). Side-by-side mapping of each traditional networking primitive (L2 learning, VLAN access/trunk, STP/RSTP, link aggregation, port mirroring, L3 routing, stateless ACL, stateful ACL plus NAT, QoS, DHCP/ARP gap, flow export) to its OVS implementation site. Architectural-plane separation table (management plus control plus data plane). Failure-isolation model. Production-readiness comparison and migration recipe for the traditional engineer.
