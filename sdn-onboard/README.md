...
- Part 4.9, [OpenFlow + OVS action catalog](4.9%20-%20openflow-action-catalog.md) *(content tier 1+2, Phase H session S42+S43; tier 3 advanced S44)*, reference 40+ action theo 7 category với Template C 8-attribute anatomy per action. Tier 1+2 hoàn thành: Output (9 action) + group 4-type + encap/decap (VLAN/MPLS/PBB) + field mod (set_field + mod_* legacy + dec_ttl + copy_ttl + move + load) + metadata (write_metadata/set_tunnel) + QoS (set_queue/enqueue/meter) + control (resubmit/clone/note) + Action Set 12-priority.
