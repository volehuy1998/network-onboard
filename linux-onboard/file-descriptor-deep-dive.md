...
**Phần C — fork(): con thừa kế toàn bộ bảng FD — khác biệt cốt lõi với dup()**
...
`open()` độc lập tạo OFD mới — bước 5 cho thấy FD 5 bắt đầu tại `pos: 0` dù FD 3/FD 4 đang ở `pos: 8`, vì mỗi `open()` tạo entry riêng trong bảng thứ hai. Đây chính là kịch bản "f_pos = 300 vs f_pos = 0" trong Figure 1-1.

`dup()` tạo thêm FD trỏ đến OFD đã có — bước 4 cho thấy đọc qua FD 4 làm FD 3 thay đổi offset, vì cả hai nhìn vào cùng OFD "A". Nhưng quan trọng: `dup()` là thao tác **chọn lọc** — suốt cả Phần A, FD 5 (OFD "B") vẫn giữ nguyên `pos: 0`, vì `dup(3→4)` chỉ ảnh hưởng OFD "A", không đụng gì đến OFD "B".

`fork()` nhân bản **toàn bộ** per-process FD table — bước 7 chứng minh điều này: con đọc qua FD 5 và parent's FD 5 nhảy từ `pos: 0` lên `pos: 3`, dù FD 5 là OFD "B" hoàn toàn tách biệt với OFD "A". Đây là điều mà `dup()` không làm được và không bao giờ xảy ra trong Phần A. `fork()` là thao tác **bán buôn**: con thừa kế mọi FD của cha — bao gồm socket lắng nghe, file nhạy cảm, pipe — bất kể cha có muốn hay không. Đây chính xác là lý do cờ CLOEXEC tồn tại (xem mục 1.10): cho phép cha đánh dấu "FD này không được thừa kế khi exec()" để ngăn FD leak sang process con.
