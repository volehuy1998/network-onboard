...
- [2.10 - Quản lý `SELinux` ( :arrow_up: UPDATED 27/01/2024)](#selinux_manage)
    - [2.10.1 - Kiến trúc `SELinux` ( :arrow_up: UPDATED 27/01/2024)](#selinux_arch)
    - [2.10.2 - Sử dụng `SELinux` cơ bản với chính sách `targeted` ( :arrow_up: UPDATED 27/01/2024)](#basic_selinux)
      - [2.10.2.1 - Xem nhãn, kích hoạt và vô hiệu hóa `SELinux` ( :arrow_up: UPDATED 27/01/2024)](#selinux_activation)
      - [2.10.2.2 - Xem định nghĩa chính sách `SELinux` ( ::heavy_plus_sign:: UPDATED 27/01/2024)](#selinux_policy_searching)
      - [2.10.2.3 - Kiểm soát `fcontext` với nhãn sẵn có ( :arrow_up: UPDATED 27/01/2024)](#basic_file_context)
      - [2.10.2.4 - Kiểm soát `port` với nhãn sẵn có ( :arrow_up: UPDATED 27/01/2024)](#basic_port)
      - [2.10.2.5 - Kiểm soát chính sách với  `boolean` ( :arrow_up: UPDATED 27/01/2024)](#basic_boolean)
...
```shell
[root@huyvl-linux-training ~]# sesearch --allow --source httpd_t --target httpd_sys_content_t -ds -dt
...
<i>Chú thích: tiến trình Apache sẽ truy cập được tất cả những tệp nhãn `httpd_user_content_type` chỉ đã kích hoạt `httpd_enable_homedirs` thông qua lệnh `setbool`. Xem thêm kiểm soát chính sách SELinux với Boolean [tại mục 2.10.2.5](#basic_boolean).</i>
