...
**5. Liên hệ concept.** Nối output với cơ chế đã học. Ví dụ: `packets`/`bytes` counter trong `ofproto/trace` không tăng cho đến khi có `-generate`, vì đây là simulation không gửi packet thật — kết nối với Part 9.2 §9.2.X fast/slow path.
...
Xem Part 9.4 session S38 expansion — section §9.4.X "Đọc hiểu output `ovs-ofctl dump-flows`" là ví dụ hoàn chỉnh áp dụng template này.
