...
**Before You Begin:**
Bạn cần một terminal Linux với quyền user thường (không cần root). Đảm bảo `/proc` filesystem đã mount (mặc định trên mọi Linux distribution).
...
HAProxy sử dụng level-triggered epoll mặc định — chế độ này đơn giản hơn và ít lỗi hơn vì `epoll_wait()` tiếp tục thông báo khi buffer còn dữ liệu chưa đọc. Từ phiên bản gần đây, HAProxy cung cấp tùy chọn thực nghiệm `tune.fd.edge-triggered on` trong `global` section để bật edge-triggered trên một số socket, nhưng tính năng này chưa phải mặc định và được đánh dấu experimental trong source code (`KWF_EXPERIMENTAL`).
