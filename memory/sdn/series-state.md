...
| 0.5 | Open vSwitch bridge and port primitives | 802 | DONE (v3.13 R2 closure, 2026-04-30) | Curriculum wrap of eight R2 lab transcripts (R2.0 orientation, R2.1 fail-mode, R2.2 L2 trace, R2.3 port types, R2.4 patch ports, R2.6 datapath choice, R2.7 OVSDB UUID, R2.8 MAC binding with empirical proofs). Four-piece architecture, four port types contrasted, two datapaths contrasted, three caches that influence forwarding, OVSDB row-UUID hierarchy, complete worked example, 12 misconceptions sourced to specific sub-captures, 10 end-of-part questions, two production diagnostic walkthroughs, six operator habits, OVN-readiness pointer, glossary, R2 source-anchor pocket reference, OVS source-tree reading order. Pairs with the eight R2 lab transcripts. Authored under owner directive of 2026-04-30 (audience-centred pedagogy per Rule 19) and "every conjecture proven by practice" directive. |
