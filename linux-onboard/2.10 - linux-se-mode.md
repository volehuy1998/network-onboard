...
- Phiên bản `RHEL9` được cài đặt sẵn `setroubleshoot-server` để cho phép xem thông tin gợi ý sửa lỗi tại `/var/log/messages`, các bản thấp hơn chưa được cài đặt.
</i>