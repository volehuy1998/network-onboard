...
Khi đóng gói với tệp thông thường thì tương tự như việc sử dụng công cụ `zip` hoặc một số biến thể ở trên các hệ điều hành khác. Về nguồn gốc, các công cụ đóng gói tệp ban đầu có phổ biến sử dụng thuật toán `PKZIP (Phil Katz's ZIP)` cho hệ thống `MSDOS`, sau đó nó được phát triển để chạy được trên `RHEL` với lệnh `zip` và `unzip`. Nhiều thuật toán nén khác được phát triển từ khi `zip` được giới thiệu và mỗi cái trong số chúng đều có ưu điểm riêng. Để đạt được mục đích sử dụng chung thì bất cứ thuật toán nén có trong công cụ `tar` đều được hỗ trợ bởi các công cụ khác.

Tệp đóng gói được sử dụng để tạo ra các bản sao lưu cá nhân, hoặc đơn giản là chuyển nó sang hệ thống khác thông qua mạng máy tính với các phương pháp như `rsync`. Việc đóng gói hoàn tòan diễn ra độc lập, có thể  kết hợp với nén để giảm kích thước.
...
Việc đóng gói thường sẽ không tiêu tốn nhiều thời gian và tài nguyên `CPU` nhưng nhược điểm là tệp đóng gói gần như giữ nguyên kích thước. Điều này làm tăng thời gian truyền tải đến hệ thống khác thông qua mạng, chiếm băng thông.
...
Lệnh `tar` hỗ trợ đường dẫn tương đối lẫn tuyệt đối. Mặc định, `tar` sẽ loại bỏ dấu `/ (forward slash)` đầu tiên ở khai báo nguồn dữ liệu (nếu có) do đó các tệp được đóng gói ở dạng tương đối. Với tính năng này sẽ tạo ra sự an toàn cho hệ thống, bởi vì việc giải nén nếu có dấu `/` ở đầu sẽ được hiểu là đường dẫn tuyệt đối và có khả năng ghi đè tệp quan trọng, nhạy cảm như `/etc/shadow`, ... Khi được đóng gói với đường dẫn tương đối thì các tệp đươc giải nén ở trọng thư mục mới mà không xảy ra tình huống ghi đè. Để tạo được tệp đóng gói thì người dùng cần quyền đọc ở nguồn được chỉ định. Ví dụ ở `/etc` yêu cầu quyền `root`, với người dùng không có quyền thì tệp đóng gói sẽ không bao gồm các tệp mà người dùng đó không có quyền đọc và các thư mục không có quyền đọc và thực thi (lệnh `cd`).
