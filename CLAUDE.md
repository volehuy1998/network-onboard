...
> **Language convention (updated 2026-04-28 per Rule 17 below).** This file (CLAUDE.md), every working file under `memory/*`, every plan file under `plans/*`, and every curriculum file under `sdn-onboard/*.md` and `haproxy-onboard/*.md` must be written in plain technical English. The audience for the curriculum is Vietnamese network engineers reading at CEFR B2 to C1 level. Style policy is defined in `memory/shared/english-style-guide.md`. Legacy Vietnamese sections that have not yet been migrated remain valid until plan v3.12 closes; files in mixed-language transition state carry a `**Language status:**` callout per plan v3.9.1 §8.3. Files under `linux-onboard/*.md` and `network-onboard/*.md` are out of the active migration scope and may stay as-is.
...
The dictionary at `memory/shared/rule-11-dictionary.md` is frozen as a historical reference for plan v3.12 legacy-section translation.
...
**One narrow scoped allowance.** The historical dictionary at `memory/shared/rule-11-dictionary.md` (which lives outside the named scope, in `memory/`) keeps its bilingual body as a translation reference for plan v3.12 legacy-section migration. Its header is in English and marks the file as frozen.

**Transition status for legacy files.** Existing Vietnamese curriculum sections in files that v3.9.1 does not modify remain valid until plan v3.12 closes. Files modified by v3.9.1 carry a `**Language status:**` callout immediately after the H1 heading per plan v3.9.1 §8.3, listing which sections are now English. Files not yet touched stay legacy Vietnamese without a marker; plan v3.12 will add markers and translations in batches.
...
| Key | Value |
|-----|-------|
| Branch | `docs/sdn-foundation-rev2`. Latest tag: `v4.0-MasteryComplete` (2026-04-26 local) plus optional `v4.0.1-OVSHotfix` (2026-04-27 local, plan v3.9 close). **No remote push per system policy.** |
| Curriculum | About 128 files in `sdn-onboard/*.md`, about 96,000 lines (after v3.9 expansions), 20 blocks. All v3.8 R0 to R6 phases complete; v3.9 OVS block hotfix S0 to S8 complete on 2026-04-27. OVS scope (Block 9, Block 10, 20.0, 20.1): zero rubric leak. Non-OVS scope (Block 13, Block 3, Block 4, README): 74 leaks, deferred to v3.10 and v3.11. |
| Active phase | **Plan v3.9.1 Phase Q-1 in progress (started 2026-04-28).** Style guide written, em-dash check installed, CLAUDE.md migrated to English. Remaining: governance-principles.md migration, dictionary header freeze, then proceed to Q0 baseline. |
...
| Active plan | `plans/sdn/v3.9.1-ovs-block-source-verify-hotfix.md` (in progress 2026-04-28). Future scheduled: v3.10 OVN block hotfix per GP-12 cadence (T+7 day from any v4.x tag), v3.11 OF block hotfix, v3.12 curriculum-wide English language migration (forward stub in v3.9.1 §8.4). |
