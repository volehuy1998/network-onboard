...
- [2.7 - Kiến trúc nhật ký hệ thống (UPDATED 10/12/2023)](#sys_log_arch)
  - [2.7.1 - Tổng quan (UPDATED 03/12/2023)](#log_overview)
  - [2.7.2 - Cách sử dụng `rsyslog` (:heavy_plus_sign:UPDATED 06/12/2023)](#rsyslog)
  - [2.7.3 - Cách sử dụng `systemd-journald` (:heavy_plus_sign:UPDATED 10/12/2023)](#journald)
  - [2.7.4 - Dong bo thoi gian (:heavy_plus_sign:UPDATED 10/12/2023)](#clock_time)
    - [2.7.4.1 - Tong quan `Network Time Protocol` (:heavy_plus_sign:UPDATED 10/12/2023)](#ntp)
    - [2.7.4.2 - Cong cu `datetimectl` (:heavy_plus_sign:UPDATED 10/12/2023)](#datetimectl)
    - [2.7.4.3 - Cau hinh `NTP` su dung `chrony` (:heavy_plus_sign:UPDATED 10/12/2023)](#chronyd)
    - [2.7.4.4 - Cau hinh `NTP` su dung `ntpd` (:heavy_plus_sign:UPDATED 10/12/2023)](#ntpd)
...
Mac dinh `systemd-journald` duoc cau hinh de khong phai luu giu tin nhan lau dai voi tuy chon `Storage=volatile`. Huong dan su dung nhu sau:
...
- `volatile`: luu tru nhat ky vao thu muc `/run/log/journal`, boi vi thu muc `/run` chi ton tai tren bo nho nen se mat khi khoi rong may.
- `persistent`: luu tru nhat ky vao thu muc `/var/log/journal`, thu muc nay nam tren o cung nen se khong bi mat moi khi khoi dong may. Neu thu muc `/var/log/journal` khong ton tai thi `systemd-journald` se tu dong tao.
- `auto`: neu thu muc `/var/log/journal` ton tai no se hoat dong theo cach `persistent`, nguoc lai se hoat dong theo `volatile`.
- `none`: he thong se xoa tat ca nhat ky

Su dung tuy chon `Storage=persistent` de co the luu giu tin nhan lau dai bang cach viet chung vao o cung:
...
Tuy chon `Storage=none` se dung viec ghi nhat ky cho du `systemd-journald` van hoat dong:
...
Luu y: tuy chon `--boot (-b)` va `--list-boots` se rat huu dung khi xem xet nguyen nhan gay loi he thong.

### <a name="clock_time"></a>Dong bo thoi gian

#### <a name="ntp"></a>Tong quan `Network Time Protocol`
Dong bo thoi gian duoc su dung ben trong mo hinh mang de duy tri su dong bo giua cac thiet bi de dam bao viec phan tich dien ra tot va don gian hoa viec khac phuc su co. Neu khong co su dong bo nay thi viec theo doi nhat ky giua cac thiet bi se rat kho khan. Mot thuat ngu pho bien de chi su chinh xac cua dong ho la he thong phan cap `Stratum`. Lien tuong den he thong nay la cau truc du lieu dang cay, nut tren cung cua cay co do chinh xac cao nhat va nut o duoi cung co do chinh xac kem nhat. 

<div style="text-align:center"><img src="../images/stratum_hierarchy.png"/></div>

Dong ho chinh xac nhat hay con goi duoc goi la `Reference clock` hoac `Stratum 0`. Thiet bi co do chinh xac `Stratum 0`  rat dat tien chi ton tai o mot so dia diem nhat dinh tren the gioi, duoc su dung truyen tin hieu `GPS`, ... `Stratum 0` chi ket noi truc tiep den `Stratum 1`, cac `Stratum` dong cap con lai co the nhan thoi gian lan nhau. Moi khi co tin hieu duoc truyen xuong thi gia tri `Stratum` se tang len `1` don vi tuong duong viec giam do chinh xac. Vi du neu mot may tinh nhan tin hieu tu `Stratum 1` thi no mang gia tri `Stratum 2`, ... muc do toi da la `Stratum 15`. Thong thuong khong co thiet bi co muc do nao thap hon `Stratum 3`. Mot may tinh khi moi khoi dong se co muc do `Stratum 16` cho den khi dong bo duoc dong ho voi may chu `NTP`.

<div style="text-align:center"><img src="../images/stratum_lively.jpg"/></div>

#### <a name="datetimectl"></a>Cach su dung `datetimectl`

Lenh `timedatectl` hien thi tong quan ve thoi gian hien tai lien quan den cai dat he thong.
...
Liet ke co so du lieu ve mui gio nhu sau:
...
`Internet Assigned Numbers Authority (IANA)` la nha cung cap co so du lieu mui gio va `timedatectl` se dua tren ten mui gio trong co so du lieu. Ten mui gio duoc hinh thanh dua tren chau luc, dai duong hoac cu the hon la ten thanh pho trong khu vuc, quoc gia. Vi du thanh pho `Asia/Ho_Chi_Minh`. Su dung cong cu `tzselect` de xac dinh ten mui gio nhu sau:
...
Tai khoan `root` co the su dung de thay doi thoi gian he thong voi tuy chon `set-timezone`.
...
Tuy chon `set-time` de thay doi gio cua he thong hien tai nhu sau:
...
#### <a name="chronyd"></a>Cau hinh `NTP` su dung `chrony`

`chronyd` cung la mot dich vu dong bo thoi gian duoc trien khai dua tren `Network Time Protocol (NTP)`. Ve mac dinh `chronyd` su dung may chu tu `NTP Pool Project` de dong bo hoa thoi gian ma khong can yeu cau cau hinh bo sung. Neu khong duoc cung cap truy cap mang toan cau thi quan tri vien can thay doi may chu `NTP`.  Tu cac phien ban `CentOS 6` tro ve truoc su dung `NTP`, cac phien ban sau nhu `CentOS 7` bat dau su dung `chronyd` de thay the `NTP`.
...
Cau hinh `chronyd` tai `/etc/chrony.conf` nhu sau, thay vi lay thoi gian tu mot may chu `server *` thi nguoi dung co nhieu su lua chon hon.
...
Kiem tra de chac chan may da duoc dong bo:
...
Dong bo lai thoi gian o vi du sau khi cai dat sai gio, tuy chon `-v` de xem chi tiet thong tin nhu sau:

```shell
[root@huyvl-linux-training ~]# timedatectl 
      Local time: Sun 2023-12-10 16:47:33 +07
  Universal time: Sun 2023-12-10 09:47:33 UTC
        RTC time: Sun 2023-12-10 02:02:22
       Time zone: Asia/Ho_Chi_Minh (+07, +0700)
     NTP enabled: yes
...
[root@huyvl-linux-training ~]# 
[root@huyvl-linux-training ~]# timedatectl
      Local time: Sun 2023-12-10 09:00:03 +07
  Universal time: Sun 2023-12-10 02:00:03 UTC
        RTC time: Sun 2023-12-10 02:00:03
       Time zone: Asia/Ho_Chi_Minh (+07, +0700)
     NTP enabled: no
NTP synchronized: no
 RTC in local TZ: no
      DST active: n/a
[root@huyvl-linux-training ~]# 
[root@huyvl-linux-training ~]# chronyc sources
...
===============================================================================
^? 203.113.174.44                3   6     1     1  +9181us[+9181us] +/-  165ms
^? ntp1.tino.org                 2   6     1     1  -6240us[-6240us] +/-  135ms
^? t1.time.sg3.yahoo.com         0   6     0     -     +0ns[   +0ns] +/-    0ns
[root@huyvl-linux-training ~]#
[root@huyvl-linux-training ~]#
[root@huyvl-linux-training ~]# timedatectl
      Local time: Sun 2023-12-10 16:48:48 +07
  Universal time: Sun 2023-12-10 09:48:48 UTC
        RTC time: Sun 2023-12-10 02:01:06
       Time zone: Asia/Ho_Chi_Minh (+07, +0700)
     NTP enabled: yes
NTP synchronized: yes
 RTC in local TZ: no
...
- `^` dang tham chieu den may chu `NTP` ngoai mang toan cau. Neu `#` la may chu `NTP` cuc bo, nguoc lai `=` la dong bo tu thiet bi ngang cap `stratum`.
- `ntp1.toni.org`, ... la dia chi cua may chu `NTP`.
- Gia tri he bat phan `377` chi ra rang nhan duoc phan hoi hop le cho tat ca tam lan truyen cuoi cung.
- `LastRx` cho biet mau tin hieu cuoi cung nhan duoc cach day bao lau, mac dinh la giay. Cac don vi thoi gian khac nhu: `m` (phut), `h` (gio), `d` (ngay), `y` (nam).

#### <a name="ntpd"></a>Cau hinh `NTP` su dung `ntpd`