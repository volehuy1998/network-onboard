...
**1.** Mở terminal, chạy lệnh `sleep` ở background để tạo một process có thể quan sát:
    ...
    [linux-node]$ sleep 300 &
    [1] 12345

**2.** Thay số PID (ví dụ: 12345). Liệt kê các file descriptor của process này:

    [linux-node]$ ls -la /proc/12345/fd/
    lrwx------ 1 user user 64 Mar 29 10:00 0 -> /dev/pts/0
    lrwx------ 1 user user 64 Mar 29 10:00 1 -> /dev/pts/0
    lrwx------ 1 user user 64 Mar 29 10:00 2 -> /dev/pts/0

    2.1. FD 0, 1, 2 đều là symbolic link trỏ đến `/dev/pts/0` — đây là pseudo-terminal của session hiện tại.
    2.2. `lrwx------` cho thấy kernel cho phép đọc và ghi trên cả ba FD.

**3.** Bây giờ thử redirect stdout của một process khác vào file:

    [linux-node]$ sleep 300 > /tmp/test_output.txt &
    [2] 12346
    [linux-node]$ ls -la /proc/12346/fd/
    lrwx------ 1 user user 64 Mar 29 10:01 0 -> /dev/pts/0
    l-wx------ 1 user user 64 Mar 29 10:01 1 -> /tmp/test_output.txt
    lrwx------ 1 user user 64 Mar 29 10:01 2 -> /dev/pts/0

    3.1. FD 1 bây giờ trỏ đến `/tmp/test_output.txt` thay vì terminal.
    3.2. Permission `l-wx------` (chỉ ghi) vì file được mở với O_WRONLY bởi shell.
    3.3. FD 0 và FD 2 vẫn trỏ đến terminal — chỉ stdout bị redirect.
...
`epoll` ra đời trong Linux kernel 2.6 (2004) với một triết lý thiết kế khác hoàn toàn: thay vì process phải nói cho kernel toàn bộ danh sách FD cần giám sát mỗi lần hỏi, process đăng ký FD một lần và kernel tự duy trì danh sách đó vĩnh viễn.
...
HAProxy sử dụng edge-triggered epoll để đạt hiệu suất tối đa — nhưng đổi lại code phải được viết cẩn thận hơn để đảm bảo không bỏ sót dữ liệu.
...
Từ Linux 2.6.23, kernel bổ sung cờ nguyên tử (atomic) cho các system call tạo FD — đặt `CLOEXEC` ngay tại thời điểm FD được tạo, không có khoảng trống:
...
Đây là lý do `O_CLOEXEC` không chỉ dành cho code ứng dụng mà còn cho code thư viện. Từ glibc 2.15 trở đi, hầu hết các hàm thư viện nội bộ đã chuyển sang dùng `O_CLOEXEC` khi mở FD tạm thời. Tuy nhiên, library của bên thứ ba có thể chưa tuân thủ — đây là nguồn FD leak tiềm ẩn mà lập trình viên cần lưu ý.

### Liên hệ thực tế: close-on-exec trong HAProxy và network server

HAProxy source code sử dụng `SOCK_CLOEXEC` trên mọi `socket()` call và `O_CLOEXEC` trên mọi `open()` call. Nginx cũng áp dụng tương tự. Đây là defensive programming — ngay cả khi phiên bản hiện tại không `fork()+exec()` chương trình bên ngoài, phiên bản tương lai hoặc plugin/module có thể làm điều đó. Đặt `CLOEXEC` từ đầu ngăn ngừa rò rỉ trước khi nó có cơ hội xảy ra.

Trên OpenStack, khi Neutron agent hoặc Nova compute `fork()+exec()` các helper process (ví dụ: `dnsmasq`, `haproxy`, `ip netns exec`), FD leak là nguồn bug thực tế. Bug [LP#1535770](https://bugs.launchpad.net/neutron/+bug/1535770) trong Neutron ghi nhận tình huống FD từ parent agent rò rỉ vào `dnsmasq` child process, gây lãng phí tài nguyên và hành vi bất thường. Giải pháp: Python 3.4+ mặc định đặt `CLOEXEC` trên tất cả FD mới tạo (PEP 446), và Neutron đã chuyển sang sử dụng `subprocess` module với `close_fds=True`.
