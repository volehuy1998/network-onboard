...
- Part 4.9, [OpenFlow + OVS action catalog](4.9%20-%20openflow-action-catalog.md) *(content tier 1, Phase H session S42; tier 2 S43 + tier 3 S44 sẽ expand)*, reference 40+ action theo 7 category với Template C 8-attribute anatomy per action. Tier 1 hoàn thành: Output (output/drop/normal/flood/all/controller/local/in_port/table) + group (all/select/indirect/fast_failover) + resubmit + clone + note + Action Set 12-priority execution order + action vs instruction foundation.
