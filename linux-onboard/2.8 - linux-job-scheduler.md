...
- [2.8 - Lập lịch chạy cho tác vụ tương lai ( :arrow_up: UPDATED 01/01/2024)](#schedule_job)
  ...
  - [2.8.4 - Ứng dụng `systemd timer` ( :heavy_plus_sign: UPDATED 01/01/2024)](#systemd_timer)
    - [2.8.4.1 - Cách sử dụng công cụ `systemd timer` ( :heavy_plus_sign: UPDATED 01/01/2024)](#systemd_timer_basic)
    - [2.8.4.2 - Quản lý loại tệp tạm thời ( :heavy_plus_sign: UPDATED 01/01/2024)](#manage_tmp_file)
      - [2.8.4.2.1 - Cách sử dụng `systemd-tmpfiles --create` ( :heavy_plus_sign: UPDATED 01/01/2024)](#tmpfiles_create)
      - [2.8.4.2.2 - Cách sử dụng `systemd-tmpfiles --clean` ( :heavy_plus_sign: UPDATED 01/01/2024)](#tmpfiles_clean)
      - [2.8.4.2.3 - Cách sử dụng `systemd-tmpfiles --remove` ( :heavy_plus_sign: UPDATED 01/01/2024)](#tmpfiles_remove)
...
Thông thường các ứng dụng sẽ hoạt động dúng cách khi tệp và thư mục tạm thời của nó tồn tại. Thêm vào đó việc xóa các tệp tạm thời trên ổ cứng là sự cần thiết để tránh khỏi các vấn đề hết dung lượng. Các bản phân phối `CentOS` sử dụng công cụ `systemd-tmpfiles` để cung cấp phương pháp cấu hình `*.conf` để quản lý tệp và thư mục tạm thời như:
