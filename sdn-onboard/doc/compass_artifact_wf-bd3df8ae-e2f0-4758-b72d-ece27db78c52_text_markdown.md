...
- **Part II — OVS Day-to-Day Operations** (Chapters A–W)
- **Part III — OpenFlow in OVS** (Chapters 5–10)
- **Part IV — Tunneling and Overlays** (Chapters 11–13)
- **Part V — Performance and Scale** (Chapters 14–16)
- **Part VI — OVN Topic Chapter** (Chapter 17)
- **Part VII — Production Operations** (Chapters 18–20)
- **Appendices**
...
- **`ovs-vswitchd`** — the forwarding daemon. It owns the OpenFlow pipeline, the megaflow cache, the userspace datapath (`netdev`), and the bond/LACP/STP/RSTP state machines.
...
- **`openvswitch.ko`** (Linux kernel datapath) — the in-kernel flow cache that actually forwards packets when the datapath type is `system`. It is increasingly supplanted by the userspace `netdev` datapath with DPDK or AF_XDP.
...
The output of `show` will list the bridge `br0` with an implicit internal port of the same name, plus the newly added `eth1`. Already at this point the switch is forwarding in **NORMAL** mode, i.e. behaving like an unmanaged L2 switch with MAC learning. To prove this without moving a single packet, run the built-in simulator:

```bash
sudo ovs-appctl ofproto/trace br0 "in_port=eth1,dl_src=aa:bb:cc:dd:ee:01,dl_dst=ff:ff:ff:ff:ff:ff"
```

The output walks the OpenFlow pipeline table-by-table and ends with a `Datapath actions:` line that tells you exactly what the kernel would do. `ofproto/trace` is the single most important OVS troubleshooting primitive and you should internalize it before reading Part III.
...
# Part II — OVS Day-to-Day Operations

*This Part is the heart of the rebuild. Each chapter opens with concept, then a hands-on lab with verified syntax, then verification and failure modes, then a self-check. Every command in this Part is drawn from docs.openvswitch.org or the upstream man pages.*
...
The `--no-wait` flag is particularly important on busy hypervisors: by default `ovs-vsctl` blocks until `ovs-vswitchd` has reloaded its configuration from the DB. Under automation you often do not need to wait, and the flag halves the latency of bulk operations.
...
A bridge is the unit of OpenFlow forwarding. Create one with `ovs-vsctl add-br br0`; it comes up with `datapath_type=system` (kernel) by default. Specify `datapath_type=netdev` for the userspace datapath that DPDK and AF_XDP require; specify `datapath_type=dummy` for test beds that never touch real interfaces.

The **fail-mode** column decides what happens when all OpenFlow controllers are unreachable. With `fail_mode=standalone` (default), the bridge behaves as an unmanaged L2 switch running MAC learning and VLAN separation. With `fail_mode=secure`, the bridge stops forwarding except under flows explicitly programmed by the operator. In production with a controller, always set `secure`:
...
Remove a bridge safely by first disabling controllers (`ovs-vsctl del-controller br0`), draining traffic at the orchestration layer, then `ovs-vsctl --if-exists del-br br0`. Do not use `--if-exists` in CI pipelines where a missing bridge is itself a bug.
...
The zero-copy claim is visible in the trace output: `ofproto/trace` shows the packet recirculating into the second bridge with no datapath hop, and `ovs-dpctl dump-flows` reveals that the kernel megaflow is a single entry spanning both bridges.
...
Three separate protocols, three separate tables, one consistent idiom. sFlow samples a configurable fraction of packets (1-in-N) plus periodic counter polls; NetFlow emits flow records on timeout; IPFIX is NetFlow's standardized successor with template negotiation.
...
Verify the agent is sending with `tcpdump -ni ethX udp port 6343` on the collector host. Rotate collectors by `ovs-vsctl set sFlow <uuid> target='"new:6343"'`; the change is atomic and no packets are lost during rotation. Clear with `ovs-vsctl clear Bridge br0 sflow` (or `netflow`, or `ipfix`).
...
The queue numbers (123, 234) are arbitrary OpenFlow queue IDs that `set_queue:` in the flow-table references. Without the OpenFlow rules, packets go to the default queue and nothing is shaped — this is the single most common QoS misconfiguration in the field.
...
**Windows and OpenBSD ports**: OVS runs on Windows with a Hyper-V extensible-switch forwarding extension; on OpenBSD and NetBSD with userspace datapaths. Both ports lack DPDK and AF_XDP. Operationally they are managed with the same `ovs-vsctl` / `ovs-ofctl` tools.
...
`ct_state` is a bitfield: `+trk` (tracked), `+new` (first packet of a new conn), `+est` (established), `+rel` (related, e.g. ICMP error), `+inv` (invalid), `+rpl` (reply direction), `+snat`, `+dnat`. Combine with `+` and `-`: `ct_state=+trk+new-inv`.
...
## Chapter 8 — Multi-table pipelines and the advanced 5-table tutorial
...
## Chapter 10 — Flow-table hygiene: monitor, replace-flows, diff-flows
...
# Part IV — Tunneling and Overlays
...
Compressed form: *Are packets arriving? → tcpdump on physical NIC. Are they entering OVS? → `dpctl/dump-flows -m` hit counters. Are the right flows matching? → `ofproto/trace`. Is the action correct? → read the Datapath actions line. Does the datapath agree? → `dpctl/dump-flows` on the actual kernel/netdev. Does the wire agree? → tcpdump on egress NIC. When all four agree and packets still drop → upstream switch.*
