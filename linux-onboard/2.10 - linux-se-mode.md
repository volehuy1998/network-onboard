...
<i>Chú thích: tiến trình Apache sẽ truy cập được tất cả những tệp nhãn `httpd_user_content_type` chỉ đã kích hoạt `httpd_enable_homedirs` thông qua lệnh `setbool`. Xem thêm kiểm soát chính sách SELinux với Boolean [tại mục 2.10.2.6](#basic_boolean).</i>
