...
- [2.7 - Kiến trúc nhật ký hệ thống ( :arrow_up: UPDATED 17/12/2023)](#sys_log_arch)
  ...
  - [2.7.4 - Đồng bộ thời gian ( :arrow_up: UPDATED 17/12/2023)](#clock_time)
    - [2.7.4.1 - Tổng quan `Network Time Protocol` ( :arrow_up: UPDATED 17/12/2023)](#ntp)
    - [2.7.4.2 - Công cụ `datetimectl` ( :heavy_plus_sign: UPDATED 10/12/2023)](#datetimectl)
    - [2.7.4.3 - Cấu hình `NTP` sử dụng `chrony` ( :heavy_plus_sign: UPDATED 17/12/2023)](#chronyd)
...
Chú thích:
   ...
   - `iburst` là một tùy chọn có thể khai báo hoặc không, tùy chọn này giúp tăng tốc đồng bộ.
   - `driftfile` chứa thông tin về độ lệch để biết rằng đồng hồ cục bộ nhanh hơn hay chậm hơn đồng hồ máy chủ.
...
- `Last sample` có 3 giá trị để hiển thị độ lệch của phép đo cuối cùng. Giá trị `-17us` gần `0` cho thấy nó có độ chính xác cao với đồng hồ tham chiếu.
- Chú ý: [hai máy chủ cuối](https://tf.nist.gov/tf-cgi/servers.cgi) có `stratum` là `0` và độ sai lệch là `0` cho thấy độ chính xác gần như tuyệt đối nhưng hệ thống không chọn chúng để đồng bộ. Nguyên nhân là vì chúng đã được cấu hình xác thực nên hệ thống không nhận được dữ liệu đúng và quy tắc của `stratum 0` là chỉ kết nối trực tiếp với các `stratum 1`, chỉ có những người dùng đăng ký với `NIST` mới có thể sử dụng.
...
Người dùng có thể sử dụng `chronyc makestep` để sử dụng chức năng "bấm" thời điểm thay vì vặn/xoay kim đồng hồ hệ thống nếu độ lệch quá lớn:
